// 20250315141416
// https://unpkg.com/cursor-effects@1.0.17/dist/browser.js
...
var cursoreffects = function(t) {
    ...
    return t.bubbleCursor = function(t) {
        ...
        function u() {
            ...
            e.height = s.clientHeight) : (e.style.position = "fixed",
        ...
        function A(t) {
            ...
            o ? (e.width = s.clientWidth,
            e.height = s.clientHeight) : (e.width = h,
            e.height = c)
        }
        function m(t) {
    ...
    t.characterCursor = function(t) {
        ...
        let s, h, c, l = t?.cursorOffset || {
            x: 0,
            y: 0
        }, a = window.innerWidth, r = window.innerHeight, d = {
            x: a / 2,
            y: a / 2
        }, u = [], A = t?.font || "15px serif", m = t?.characterLifeSpanFunction || function() {
        ...
        function x() {
            ...
            e ? (s.style.position = "absolute",
            n.appendChild(s),
            s.width = n.clientWidth,
            s.height = n.clientHeight) : (s.style.position = "fixed",
        ...
        function E(t) {
            ...
            e ? (s.width = n.clientWidth,
            s.height = n.clientHeight) : (s.width = a,
            s.height = r)
        }
        function M(t) {
            if (t.touches.length > 0)
    ...
    t.clockCursor = function(t) {
        ...
        function k() {
            !function() {
                W[0] = Math.round(R[0] += (l.y - R[0]) * m),
                H[0] = Math.round(Y[0] += (l.x - Y[0]) * m);
                for (let t = 1; t < z; t++)
                    W[t] = Math.round(R[t] += (W[t - 1] - R[t]) * m),
                    H[t] = Math.round(Y[t] += (H[t - 1] - Y[t]) * m),
                    R[t - 1] >= c - 80 && (R[t - 1] = c - 80),
            ...
            function() {
                ...
                for (let t = 0; t < F.length; t++)
                    F[t].y = R[t] + 67.5 * Math.sin(-i + t * b * Math.PI / 180),
                    F[t].x = Y[t] + 67.5 * Math.cos(-i + t * b * Math.PI / 180),
                    n.fillStyle = F[t].color,
                    n.fillText(F[t].value, F[t].x, F[t].y);
                for (let t = 0; t < T.length; t++)
                    T[t].y = R[F.length + t] + 45 * Math.sin(t * B * Math.PI / 180),
                    T[t].x = Y[F.length + t] + 45 * Math.cos(t * B * Math.PI / 180),
                    n.fillStyle = T[t].color,
                    n.fillText(T[t].value, T[t].x, T[t].y);
                for (let t = 0; t < D.length; t++)
                    D[t].y = R[F.length + E + t] + 0 + t * S * Math.sin(a),
                    D[t].x = Y[F.length + E + t] + 0 + t * S * Math.cos(a),
                    n.fillStyle = D[t].color,
                    n.fillText(D[t].value, D[t].x, D[t].y);
                for (let t = 0; t < X.length; t++)
                    X[t].y = R[F.length + E + D.length + t] + 0 + t * S * Math.sin(s),
                    X[t].x = Y[F.length + E + D.length + t] + 0 + t * S * Math.cos(s),
                    n.fillStyle = X[t].color,
                    n.fillText(X[t].value, X[t].x, X[t].y);
                for (let t = 0; t < I.length; t++)
                    I[t].y = R[F.length + E + D.length + X.length + t] + 0 + t * S * Math.sin(i),
                    I[t].x = Y[F.length + E + D.length + X.length + t] + 0 + t * S * Math.cos(i),
    ...
    t.emojiCursor = function(t) {
        ...
        function g() {
            ...
            n ? (d.style.position = "absolute",
            i.appendChild(d),
            d.width = i.clientWidth,
            d.height = i.clientHeight) : (d.style.position = "fixed",
        ...
        function f(t) {
            o = window.innerWidth,
            s = window.innerHeight,
            n ? (d.width = i.clientWidth,
            d.height = i.clientHeight) : (d.width = o,
            d.height = s)
        }
        function p(t) {
            if (t.touches.length > 0)
        ...
        function y(t) {
            t.timeStamp - l < 16 || window.requestAnimationFrame(( () => {
                if (n) {
                    ...
                    h.x = t.clientX,
                    h.y = t.clientY;
                Math.hypot(h.x - c.x, h.y - c.y) > 1 && (v(h.x, h.y, r[Math.floor(Math.random() * e.length)]),
                c.x = h.x,
                c.y = h.y,
                l = t.timeStamp)
            }
    ...
    t.fairyDustCursor = function(t) {
        ...
        function m() {
            ...
            r.height = i.clientHeight) : (r.style.position = "fixed",
            i.appendChild(r),
            r.width = o,
            r.height = s),
            d.font = "21px serif",
        ...
        function g(t) {
            o = window.innerWidth,
            s = window.innerHeight,
            n ? (r.width = i.clientWidth,
            r.height = i.clientHeight) : (r.width = o,
            r.height = s)
        }
        function f(t) {
            if (t.touches.length > 0)
        ...
        function p(t) {
            window.requestAnimationFrame(( () => {
                if (n) {
                    ...
                    h.x = t.clientX,
                    h.y = t.clientY;
                Math.hypot(h.x - c.x, h.y - c.y) > 1.5 && (y(h.x, h.y, a[Math.floor(Math.random() * e.length)]),
                c.x = h.x,
                c.y = h.y)
            }
            ))
    ...
    t.followingDotCursor = function(t) {
        ...
        (s / 2,h / 2,10,10), a = t?.color || "#323232a6";
        ...
        function d() {
            ...
            e.style.left = "0px",
            e.style.pointerEvents = "none",
            i ? (e.style.position = "absolute",
            o.appendChild(e),
            e.width = o.clientWidth,
            e.height = o.clientHeight) : (e.style.position = "fixed",
            document.body.appendChild(e),
            e.width = s,
            e.height = h),
        ...
        function u(t) {
            ...
            i ? (e.width = o.clientWidth,
            e.height = o.clientHeight) : (e.width = s,
            e.height = h)
        }
        function A(t) {
            if (i) {
    ...
    t.ghostCursor = function(t) {
        ...
        let c, l, a, r = window.innerWidth, d = window.innerHeight, u = {
            x: r / 2,
            y: r / 2
        }, A = [], m = new Image;
        t && t.image ? m.src = t.image : m.src = "data:image/png;base64,";
        ...
        function f() {
            ...
            c.height = n.clientHeight) : (c.style.position = "fixed",
        ...
        function p(t) {
            ...
            e ? (c.width = n.clientWidth,
            c.height = n.clientHeight) : (c.width = r,
            c.height = d)
        }
        function y(t) {
            if (t.touches.length > 0)
    ...
    t.snowflakeCursor = function(t) {
        let e, n, i, o = t && t.element, s = o || document.body, h = ["❄️"], c = window.innerWidth, l = window.innerHeight, a = {
            x: c / 2,
            y: c / 2
        }, r = [], d = [];
        ...
        function m(t) {
            ...
            o ? (e.width = s.clientWidth,
            e.height = s.clientHeight) : (e.width = c,
            e.height = l)
        }
        function g(t) {
            if (t.touches.length > 0)
    ...
    t.springyEmojiCursor = function(t) {
        let e, n, i, o, s = t && t.emoji || "🤪", h = t && t.element, c = h || document.body, l = window.innerWidth, a = window.innerHeight, r = {
            x: l / 2,
            y: l / 2
        }, d = [];
        const u = window.matchMedia("(prefers-reduced-motion: reduce)");
        function A() {
            ...
            e.height = c.clientHeight) : (e.style.position = "fixed",
            document.body.appendChild(e),
            e.width = l,
            e.height = a),
            n.font = "16px serif",
        ...
        function m(t) {
            ...
            h ? (e.width = c.clientWidth,
            e.height = c.clientHeight) : (e.width = l,
            e.height = a)
        }
        function g(t) {
            if (t.touches.length > 0)
        ...
        function p() {
            !function() {
                ...
                for (let t = 1; t < 7; t++) {
                    ...
                    t > 0 && w(t - 1, t, i),
                    t < 6 && w(t + 1, t, i);
                    let o, s, h = new v(10 * -d[t].velocity.x,10 * -d[t].velocity.y), c = new v((i.X + h.X) / 1,(i.Y + h.Y) / 1 + 50);
                    d[t].velocity.x += .01 * c.X,
                    d[t].velocity.y += .01 * c.Y,
                    Math.abs(d[t].velocity.x) < .1 && Math.abs(d[t].velocity.y) < .1 && Math.abs(c.X) < .1 && Math.abs(c.Y) < .1 && (d[t].velocity.x = 0,
                    d[t].velocity.y = 0),
                    d[t].position.x += d[t].velocity.x,
                    d[t].position.y += d[t].velocity.y,
                    o = e.clientHeight,
                    s = e.clientWidth,
                    d[t].position.y >= o - 11 - 1 && (d[t].velocity.y > 0 && (d[t].velocity.y = .7 * -d[t].velocity.y),
                    d[t].position.y = o - 11 - 1),
                    d[t].position.x >= s - 11 && (d[t].velocity.x > 0 && (d[t].velocity.x = .7 * -d[t].velocity.x),
                    d[t].position.x = s - 11 - 1),
                    d[t].position.x < 0 && (d[t].velocity.x < 0 && (d[t].velocity.x = .7 * -d[t].velocity.x),
                    d[t].position.x = 0),
                    d[t].draw(n)
                }
            }(),
    ...
    t.textFlag = function(t) {
        ...
        function v() {
            ...
            e.height = h.clientHeight) : (e.style.position = "fixed",
            document.body.appendChild(e),
            e.width = g,
            e.height = f),
        ...
        function w(t) {
            ...
            s ? (e.width = h.clientWidth,
            e.height = h.clientHeight) : (e.width = g,
            e.height = f)
        }
        function x(t) {
            if (s) {
